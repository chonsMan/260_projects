...
private:
    struct Node {
        Node(T data, List<T>::Node* next) : data(std::move(data)), next(next) {}; //constructor
        ...
        //**********************************************************************//
        T * find(std::function<bool (T const &)> predicate) { // Trying to find the element where this function returns true
            if (predicate(data)) return & data; // if found, return address of data
            if (!next) return nullptr; // if at end and not found
            return next->find(predicate); // use recursion to find next argument
        }
...
public:
    ...
    void push_front(T data) {
        head = new Node { std::move(data), head };//if head is NULL, then list auto terminates
    }
        ...
        while(iter) {
            if (iter->data == rhs){ //if we found what we are looking for
                *prev = iter->next; //deref. prev and set it equal to the next node after iter
                ...
                T data = std::move(iter->data);  //data equals data inside current element
    ...
    //**********************************************************************//
    T * find(std::function<bool (T const &)> predicate){
        if (!head) {
            return nullptr;
        }
        return head->find(predicate);
    }
